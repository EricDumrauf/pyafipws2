...
def escribir(facts, db, schema={}, commit=True):
    ...
    try:
        for dic in facts:
            ...
            query = (
                "INSERT INTO %(detalle)s (%%(id)s, %%%%s) VALUES (?, %%%%s)" % tablas
            ) % campos["detalle"]
            ...
            if "cbtes_asoc" in dic and tablas["cmp_asoc"]:
                query = (
                    "INSERT INTO %(cmp_asoc)s (%%(id)s, %%%%s) VALUES (?, %%%%s)"
                    % tablas
                ) % campos["cmp_asoc"]
            ...
            if "permisos" in dic:
                query = (
                    "INSERT INTO %(permiso)s (%%(id)s, %%%%s) VALUES (?, %%%%s)"
                    % tablas
                ) % campos["permiso"]
            ...
            if "tributos" in dic:
                query = (
                    "INSERT INTO %(tributo)s (%%(id)s, %%%%s) VALUES (?, %%%%s)"
                    % tablas
                ) % campos["tributo"]
            ...
            if "ivas" in dic:
                query = (
                    "INSERT INTO %(iva)s (%%(id)s, %%%%s) VALUES (?, %%%%s)" % tablas
                ) % campos["iva"]
...
def modificar(fact, db, schema={}, webservice="wsfev1", ids=None, conf_db={}):
    ...
    for k in ["reproceso", "motivo_obs", "err_code", "err_msg"]:
        if "null" in conf_db and k in fact and fact[k] == None or fact[k] == "":
            ...
            fact[k] = NULL
    try:
        query = ("UPDATE %(encabezado)s SET %%%%s WHERE %%(id)s=?" % tablas) % campos[
            "encabezado"
        ]
        fields = [
...
def leer(db, schema={}, webservice="wsfev1", ids=None, **kwargs):
    ...
    if kwargs:
        ...
        query = (
            "SELECT * FROM %(encabezado)s WHERE (%%(resultado)s IS NULL OR %%(resultado)s='' OR %%(resultado)s=' ') AND (%%(id)s IS NOT NULL) AND %%(webservice)s=? ORDER BY %%(tipo_cbte)s, %%(punto_vta)s, %%(cbte_nro)s"
            % tablas
        ) % campos["encabezado"]
        ids = [webservice]
    else:
        query = ("SELECT * FROM %(encabezado)s WHERE " % tablas) + " OR ".join(
            ["%(id)s=?" % campos["encabezado"] for id in ids]
        )
    if DEBUG:

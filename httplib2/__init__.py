...
a string that contains the response entity body.
        ...
        try:
            ...
            #El siguiente campo fue comentado porque no me permitia realizar la consulta (dato tipo byte y debía ser str)
            #uri = uri.replace(" ", "%20").replace("\r", "%0D").replace("\n", "%0A")

            # El siguiente campo fue comentado porque no me permitia realizar la consulta (dato tipo byte y debía ser str)
            #(scheme, authority, request_uri, defrag_uri) = urlnorm(uri)
            (scheme, authority, request_uri, defrag_uri) = urlnorm("https://dfe.arba.gov.ar/DomicilioElectronico/SeguridadCliente/dfeServicioConsulta.do")

...
class IIBB(object):
    ...
    def ConsultarContribuyentes(self, fecha_desde, fecha_hasta, cuits_contribuyentes):
        "Realiza la consulta remota a ARBA, estableciendo los resultados"
        self.limpiar()
        try:
            for contribuyente in cuits_contribuyentes:
                self.xml = SimpleXMLElement(XML_ENTRADA_BASE)
                self.xml.fechaDesde = fecha_desde
                self.xml.fechaHasta = fecha_hasta
                self.xml.contribuyentes.contribuyente.cuitContribuyente = cuits_contribuyentes[contribuyente]
                xml = self.xml.as_xml()
                    ...
                    if "contribuyentes" in self.xml:
                        for contrib in self.xml.contribuyente:
                            c = {
                                ...
                                "NumeroComprobante": self.NumeroComprobante,
                                "Errores": [],
                            }

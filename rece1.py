...
def autorizar(ws, entrada, salida, informar_caea=False):
    ...
    if "--testing" in sys.argv:
        ...
        cbte_nro = (
            int(
                ws.CompUltimoAutorizado(
                    encabezado["tipo_cbte"], encabezado["punto_vta"]
                )
            )
            + 1
        )
...
def main():
    ...
    if config.has_section("PROXY") and not HOMO:
        ...
        proxy_dict = {}
    CACERT = (
        config.has_option("WSFEv1", "CACERT") and config.get("WSFEv1", "CACERT") or None
    )
    WRAPPER = (
        config.has_option("WSFEv1", "WRAPPER")
        and config.get("WSFEv1", "WRAPPER")
        or None
    )
    ...
    try:
        ...
        if "/prueba" in sys.argv:
            ...
            concepto = 3
            tipo_doc = 80
            nro_doc = "30500010912" if "--fce" in sys.argv else "50000000016"
            cbt_desde = cbte_nro + 1
            cbt_hasta = cbte_nro + 1
            imp_total = "1220000.00"
            imp_tot_conc = "0.00"
            imp_neto = "1000000.00"
            imp_iva = "210000.00"
            imp_trib = "10000.00"

...
class TestIssues(unittest.TestCase):
    ...
    def test_ajuste_unificado(self):
        ...
        try:
            ...
            self.assertEqual(
                wslpg.GetParametro("deducciones", 0, "importe_iva"), "1.05"
            )
            ...
            self.assertEqual(
                wslpg.GetParametro("retenciones", 0, "importe_retencion"), "10.50"
            )
    ...
    def test_ajuste_contrato(self, nro_contrato=27):
        ...
        try:
            ...
            self.assertEqual(float(wslpg.GetParametro("operacion_con_iva")), 0.00)
            self.assertEqual(
                float(wslpg.GetParametro("deducciones", 0, "importe_iva")), 10.50
            )
            self.assertEqual(
                float(wslpg.GetParametro("deducciones", 0, "importe_deduccion")), 110.50
            )
    ...
    def test_asociaciar_coe_contrato(self, nro_contrato=27):
        ...
        try:
            ...
            wslpg.AsociarLiquidacionAContrato(
                coe=coe,
                nro_contrato=nro_contrato,
                cuit_comprador="20400000000",
                cuit_vendedor="23000000019",
            ...
            try:
                ...
                self.assertEqual(
                    wslpg.Errores[0],
                    "2100: El contrato ingresado no se encuentra registrado.",
                )
                pass
    ...
    def test_consultar_ajuste_unificado(self):
        ...
        self.assertEqual(
            float(wslpg.GetParametro("deducciones", 0, "importe_iva")), 1.05
        )
        # verificar ajuste debito
        ok = wslpg.AnalizarAjusteDebito()
        self.assertTrue(ok)
        ...
        self.assertEqual(
            float(wslpg.GetParametro("retenciones", 0, "importe_retencion")), 10.50
        )

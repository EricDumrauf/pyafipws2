...
class TrazaVet(BaseWS):
    ...
    @inicializar_y_capturar_excepciones
    def SendConfirmaTransacc(
        self, usuario, password, p_ids_transac, f_operacion, n_cantidad=None
    ):
    ...
    def GetTransacciones(
        ...
        # llamo al webservice
        res = self.client.getTransacciones(arg0=usuario, arg1=password, **kwargs)
        ret = res["return"]
        if ret:
            self.__analizar_errores(ret)
...
def main():
    ...
    errores = []
    formatos = [
        ("TransaccionDTO", TRANSACCION_DTO, transaccion_dto),
    ...
    if "--test" in sys.argv:
        transaccion_dto.append(
            dict(
                ...
                f_vto=(datetime.datetime.now() + datetime.timedelta(30)).strftime(
                    "%d/%m/%Y"
                ),
                id_evento=11,
    ...
    if "--confirma" in sys.argv:
        ...
        ws.GetTransacciones(*sys.argv[sys.argv.index("--consulta") + 1 :])

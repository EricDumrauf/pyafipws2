...
class WSCOC(object):
    ...
    _public_attrs_ = [
        ...
        "MontoPesos",
        "CUITRepresentante",
        "DenominacionRepresentante",
        "TipoDoc",
        "NumeroDoc",
        "CUITConsultada",
        "DenominacionConsultada",
        "DJAI",
        "CodigoExcepcionDJAI",
        "DJAS",
        "CodigoExcepcionDJAS",
    ...
    def __analizar_solicitud(self, det):
        ...
        self.CUITComprador = str(det.get("detalleCUITComprador", {}).get("cuit", ""))
        self.DenominacionComprador = det.get("detalleCUITComprador", {}).get(
            "denominacion"
        )
        self.CodigoMoneda = det.get("codigoMoneda")
        self.CotizacionMoneda = det.get("cotizacionMoneda")
        self.MontoPesos = det.get("montoPesos")
        self.CUITRepresentante = str(
            det.get("DetalleCUITRepresentante", {}).get("cuit", "")
        )
        self.DenominacionRepresentante = det.get("DetalleCUITRepresentante", {}).get(
            "denominacion"
        )
    ...
    def Conectar(
        ...
        self.client.services["COCService"]["ports"]["COCServiceHttpSoap11Endpoint"][
            "location"
        ] = location
    ...
    @inicializar_y_capturar_excepciones
    def GenerarSolicitudCompraDivisa(
        self,
        cuit_comprador,
        codigo_moneda,
        cotizacion_moneda,
        ...
        codigo_destino,
        djai=None,
        codigo_excepcion_djai=None,
        djas=None,
        codigo_excepcion_djas=None,
        tipo=None,
        codigo=None,
    ):
        ...
        res = self.client.generarSolicitudCompraDivisa(
            ...
            djai=djai,
            codigoExcepcionDJAI=codigo_excepcion_djai,
            djas=djas,
            codigoExcepcionDJAS=codigo_excepcion_djas,
    ...
    @inicializar_y_capturar_excepciones
    def GenerarSolicitudCompraDivisaTurExt(
        self,
        tipo_doc,
        numero_doc,
        apellido_nombre,
    ...
    def ConsultarCUIT(self, numero_doc, tipo_doc=80, sep="|"):
        ...
        if "consultarCUITReturn" in res:
            ...
            if "tipoNumeroDoc" in ret:
                ...
                return [
                    (u"%(cuit)s\t%(denominacion)s" % d["detalleCUIT"]).replace(
                        "\t", sep
                    )
    ...
    @inicializar_y_capturar_excepciones
    def ConsultarSolicitudesCompraDivisas(
        self,
        cuit_comprador,
        estado_solicitud,
        fecha_emision_desde,
    ...
    def ConsultarMonedas(self, sep="|"):
        ...
        return [
            ("%(codigo)s\t%(descripcion)s" % p["codigoDescripcion"]).replace("\t", sep)
    ...
    def ConsultarTiposEstadoSolicitud(self, sep="|"):
        ...
        return [
            ("%(codigo)s\t%(descripcion)s" % p["codigoDescripcionString"]).replace(
                "\t", sep
            )
...
def main():
    ...
    if "--prueba" in sys.argv:
        ...
        try:
            ...
            if not "--tur" in sys.argv:
                ...
                print("djai", djai)
                ok = ws.GenerarSolicitudCompraDivisa(
                    cuit_comprador,
                    codigo_moneda,
                    cotizacion_moneda,
                    monto_pesos,
                    ...
                    codigo_destino,
                    djai=djai,
                    codigo_excepcion_djai=cod_ex_djai,
                    djas=djas,
                    codigo_excepcion_djas=cod_ex_djas,
                    tipo=tipo,
                    codigo=codigo,
                )
            else:
    ...
    if "--consultar_solicitudes" in sys.argv:
        ...
        sols = ws.ConsultarSolicitudesCompraDivisas(
            cuit_comprador,
            estado_solicitud,
            fecha_emision_desde,
            fecha_emision_hasta,
        ...
        while ws.LeerSolicitudConsultada():
            print("-" * 80)
            coc = ws.ObtenerTagXml(
                "arrayDetallesSolicitudes", "detalleSolicitudes", i, "coc"
            )
            cuit = ws.ObtenerTagXml(
                "arrayDetallesSolicitudes", "detalleSolicitudes", i, "cuit"
            )
    ...
    if "--parametros" in sys.argv:
        ...
        print(
            u"\n".join(
                ["||%s||" % s for s in ws.ConsultarTiposEstadoSolicitud(sep="||")]
            )
        )
        print("=== Monedas ===")
        ...
        print(
            u"\n".join(
                ["||%s||" % s for s in ws.ConsultarMotivosExcepcionDJAI(sep="||")]
            )
        )
        ...
        print(
            u"\n".join(
                ["||%s||" % s for s in ws.ConsultarMotivosExcepcionDJAS(sep="||")]
            )
        )

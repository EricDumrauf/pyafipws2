    ...
    try:
        ...
        if "--prueba" in sys.argv or "--formato" in sys.argv:
            ...
            if [
                ...
                if argv.startswith(("--confirmar", "--regresar", "--cambiar"))
            ]:
                prueba.update(
                    dict(
                        numero_ctg="49241727",
                        transaccion="10000001681",
                        consumo_propio="S",
                    )
                )
            parcial = dict(
        ...
        if "--solicitar" in sys.argv:
            ...
            for it in items:
                print(
                    "solicitando...",
                    " ".join(["%s=%s" % (k, v) for k, v in list(it.items())]),
                )
        ...
        if "--parcial" in sys.argv:
            ...
            for it in items:
                print(
                    "solicitando dato pendiente...",
                    " ".join(["%s=%s" % (k, v) for k, v in list(parcial.items())]),
                )
                ctg = wsctg.SolicitarCTGDatoPendiente(
                    numero_carta_de_porte=wsctg.CartaPorte, **parcial
                )
                print("numero CTG: ", ctg)
                print("Observiacion: ", wsctg.Observaciones)
                print("Carta Porte", wsctg.CartaPorte)
        ...
        if "--consultar_excel" in sys.argv:
            archivo = input("Archivo a generar (planilla.xls): ") or "planilla.xls"
            wsctg.LanzarExcepciones = True
            ok = wsctg.ConsultarCTGExcel(
                fecha_emision_desde="01/04/2012", archivo=archivo
            )
            print("Errores:", wsctg.Errores)
        ...
        if "--consultar_constancia_pdf" in sys.argv:
            ...
            if len(sys.argv) > i + 2 and not sys.argv[i + 1].startswith("--"):
                ...
                archivo = (
                    input("Archivo a generar (constancia.pdf): ") or "constancia.pdf"
                )

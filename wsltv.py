...
class WSLTV(BaseWS):
    ...
    def Conectar(
        ...
        if ok:
            ...
            location = self.client.services["LtvService"]["ports"]["LtvEndPoint"][
                "location"
            ]
            if location.startswith("http://"):
                ...
                self.client.services["LtvService"]["ports"]["LtvEndPoint"][
                    "location"
                ] = location
                print(location)
        return ok
    ...
    def AgregarFardo(self, cod_trazabilidad, clase_tabaco, peso, **kwargs):
        ...
        fardo = dict(
            codTrazabilidad=cod_trazabilidad, claseTabaco=clase_tabaco, peso=peso
        )
        self.solicitud["romaneo"][-1]["fardo"].append(fardo)
        return True

    @inicializar_y_capturar_excepciones
    def AgregarPrecioClase(
        self, clase_tabaco, precio, total_kilos=None, total_fardos=None, **kwargs
    ):
        "Agrego un PrecioClase a la liq."
        precioclase = dict(
            claseTabaco=clase_tabaco,
            precio=precio,
            totalKilos=total_kilos,
            totalFardos=total_fardos,
        )
    ...
    @inicializar_y_capturar_excepciones
    def AgregarTributo(
        self, codigo_tributo, descripcion, base_imponible, alicuota, importe
    ):
        "Agrega la información referente a las retenciones de la liquidación"
        trib = dict(
            codigoTributo=codigo_tributo,
    ...
    def AnalizarLiquidacion(self, liq):
        ...
        if liq:
            ...
            self.params_out = dict(
                ...
                cod_provincia_origen_tabaco=liq["datosOperacion"].get(
            ...
            for romaneo in liq["detalleOperacion"].get("romaneo", []):
                self.params_out["romaneos"].append(
                    dict(
            ...
            for trib in liq.get("tributo", []):
                self.params_out["tributos"].append(
                    dict(
    ...
    def ConsultarLiquidacion(
        self, tipo_cbte=None, pto_vta=None, nro_cbte=None, cae=None, pdf="liq.pdf"
    ):
        ...
        if sep is None:
            ...
            return dict(
                [
                    (
                        it["codigo"],
                        {"descripcion": it["descripcion"], "clases": it["clase"]},
                    )
                    for it in array
                ]
            )
            ...
            for it in array:
                for clase in it["clase"]:
                    ret.append(
                        ("%s %%s %s %%s %s %%s %s %%s %s" % (sep, sep, sep, sep, sep))
                        % (
                            it["codigo"],
                            it["descripcion"],
                            clase["descripcion"],
...
def main():
    ...
    try:
        ...
        PDF = (
            config.has_option("WSLTV", "PDF")
            and config.get("WSLTV", "PDF")
            or "liq.pdf"
        )
        ...
        wsltv.Conectar(
            url=WSLTV_URL, proxy=PROXY, wrapper=WRAPPER, cacert=CACERT, timeout=TIMEOUT
        )
        ...
        if "--autorizar" in sys.argv:
            ...
            if "--testing" in sys.argv:
                ...
                assert (
                    wsltv.GetParametro("romaneos", 0, "detalle_clase", 0, "cod_clase")
                    == "4"
                )
                assert (
                    wsltv.GetParametro("romaneos", 0, "detalle_clase", 0, "importe")
                    == "171000.0"
                )
                assert (
                    wsltv.GetParametro(
                        "romaneos", 0, "detalle_clase", 0, "peso_fardos_kg"
                    )
                    == "900"
                )
                assert (
                    wsltv.GetParametro(
                        "romaneos", 0, "detalle_clase", 0, "precio_x_kg_fardo"
                    )
                    == "190.0"
                )
                assert wsltv.GetParametro("romaneos", 0, "nro_romaneo") == "321"
                assert (
                    wsltv.GetParametro("romaneos", 0, "fecha_romaneo") == "2015-12-10"
                )
        ...
        if "--generar-ajuste-fisico" in sys.argv:
            if "--prueba" in sys.argv:
                ...
                wsltv.AgregarComprobanteAAjustar(
                    tipo_cbte=151, pto_vta=3697, nro_cbte=2
                )
        ...
        if "--ajustar" in sys.argv:
            if "--prueba" in sys.argv:
                ...
                wsltv.AgregarComprobanteAAjustar(
                    tipo_cbte=151, pto_vta=4521, nro_cbte=12345678
                )
                wsltv.AgregarPrecioClase(
                    clase_tabaco=111, precio=25, total_kilos=41, total_fardos=1
                )
        ...
        if "--consultar" in sys.argv:
            ...
            if "--mostrar" in sys.argv and pdf:
                wsltv.MostrarPDF(archivo=pdf, imprimir="--imprimir" in sys.argv)
